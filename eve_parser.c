...
ejday(unsigned int year, unsigned int month, unsigned int day)
{
	...
	return (year*365 + year/4 - year/100 + year/400
	    + (month * 306 + 5) / 10 + day - 1 - E_JDAY) * DAYSECONDS;
}
...
pt_to_utc(const uint32_t pacificTime)
{
	...
	* the range of dates that we care about. */
	...
	if (pacificTime < D2006040203) {
		return pacificTime + 8 * HOURSECONDS;
	} else if (pacificTime < D2006102901) {
		return pacificTime + 7 * HOURSECONDS;
...
parse_datetime(const char **s)
{
	...
	*s += 5;  /* Skip the '-' */
	...
	/* 'HH:' */
	val += (((*s)[0] - '0') * 10 + ((*s)[1] - '0')) * HOURSECONDS; *s += 3;
	val += (((*s)[0] - '0') * 10 + ((*s)[1] - '0')) * MINSECONDS; *s += 3;
...
static int
has_bad_value(const struct Raw_Record *rec)
{
